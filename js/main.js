...
function stop(which_you) {
    ...
    which_computer = options[Math.floor(Math.random()*15000)%3];
...
function increment(who) {
	count = count+1;
	if (who == "you"){
    score = parseInt($("#score_you").html())+1;
    $("#score_you").html(score);
}
else {
scorez = parseInt($("#score_computer").html())+1;
    $("#score_computer").html(scorez);
}
if (count == "5" ) {
	//alert ("YOU WIN");
...
function populateDB(tx) {
     ...
     tx.executeSql('CREATE TABLE IF NOT EXISTS DEMO (id,  [data] INTEGER  NOT NULL)');
     tx.executeSql('INSERT INTO DEMO (id, data) VALUES ("scoreyous", '+score+')');
     tx.executeSql('INSERT INTO DEMO (id, data) VALUES ("scorecoms", '+scorez+')');
}
...
//document.location.href = "win.html";
	...
	if (score > scorez)
		{
			alert ("YOU WIN"+score+"--"+scorez);
			document.location.href = "wins.html";
		}
	else
		{
			alert ("YOU LOST");
